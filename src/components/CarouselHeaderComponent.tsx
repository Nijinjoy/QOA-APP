...
const CarouselHeaderComponent = (props) => {
    ...
    return (
        <ImageBackground source={{ uri: backgroundImage }} /* source={backgroundImage} */ style={{ width: WIDTH, height: HEIGHT * 0.3 }} resizeMode='cover' >
            <View style={{ marginHorizontal: WIDTH * 0.05, position: 'absolute', bottom: HEIGHT * 0.02 }}>
                <Pressable onPress={() => navigation.goBack()} style={{ width: WIDTH * 0.1, justifyContent: 'center', alignItems: 'center', height: HEIGHT * 0.052, borderRadius: WIDTH * 0.02, position: "absolute", bottom: HEIGHT * 0.18, backgroundColor: backgroundcolor, left: WIDTH * 0.0 }}>
